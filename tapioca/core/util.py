...
class BlockRegistry():
  ...
  def register(self, block):
    ...
    This can be somewhat computationally intensive as the binary block will be
    hashed.

    If a parent block registry was provided at construction, the block will also
    be registered with the parent registry.
...
class ManifestFactory():
  ...
  def __init__(self, block_registry=None, blocks_processors=None):
    self.block_registry = block_registry
    self.block_processors = [] if block_processors is None else block_processors
...
def validate_block(block, manifest_block):
  return manifest_block.size == len(block) and \
         manifest_block.hash == hash_block(block)
